...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const user = (await pg`select * from users where email = ${decoded.email}`)[0];
    // valid token
    // console.log(user, decoded)
    if (!user) {
      // create a user
      pg`INSERT into users (email) values (${decoded.email});`.then(console.log);
    }
    const sessionToken = jwt.sign({ email: decoded.email }, process.env.JWT_SECRET, {
      expiresIn: "30d"
    });
